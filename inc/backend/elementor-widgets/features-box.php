...
class Theratio_Features_Box extends Widget_Base{
	...
	protected function render() {
		...
		if ( ! empty( $settings['link']['url'] ) ) {
			...
			$title_html = sprintf( '<%1$s %2$s><a %3$s>%4$s</a></%1$s>', $settings['heading_size'], $this->get_render_attribute_string( 'heading' ), $this->get_render_attribute_string( 'features' ), $title);
		}
		?>
